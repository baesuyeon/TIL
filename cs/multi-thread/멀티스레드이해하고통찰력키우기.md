...
- async 함수의 리턴 값은 무조건 Promise이다.
  - async 함수를 실행시킨 뒤에는 then 과 catch 를 활용하여 흐름을 제어할 수 있다.
- async 함수 안에서만 await 키워드를 사용할 수 있다. (동기 환경에서 비동기 작업을 마냥 기다리는 건 의미가 없다)
  - await은 Promise 가 resolve한 값을 내놓고 reject가 발생한다면 예외가 발생한다. (async 함수 내에서 then, catch 메소드의 존재를 잊게 할 수 있다.)
- 코드가 동기처럼 읽혀서 훨씬 직관적이다.
